...
addr_t alloc_mem(uint32_t size, struct pcb_t * proc) {
        ...
        if ( (num_pages_avail >= num_pages) && (proc->pc + num_pages*PAGE_SIZE) < (1 << ADDRESS_SIZE) )
                mem_avail = 1;
        ...
        if (mem_avail) {
                ...
                */
                for (int current_page = 0; current_page < NUM_PAGES; current_page++) {
                        if (_mem_stat[current_page].proc == 0 && current_trans < num_pages){
                                _mem_stat[current_page].proc = proc->pid;
                                _mem_stat[current_page].index = current_trans;
                                _mem_stat[current_page].next = -1;
                                if (pre_page != -1)
                                        _mem_stat[pre_page].next = current_page;
                                ...
                                trans_table->table[trans_table->size].p_index = current_page;
                                trans_table->size++;
...
int free_mem(addr_t address, struct pcb_t * proc) {
        ...
        if (translate(address, &physical_addr, proc)) {
                ...
                while (physical_addr != -1) {
                        ...
                        //_ram[(physical_addr << OFFSET_LEN) + address] = 0;
                        physical_addr = _mem_stat[physical_addr].next;

                        //_ram[physical_addr] = 0;
                        addr_t first_lv = get_first_lv(address);
                        addr_t second_lv = get_second_lv(address);
                        struct trans_table_t * trans_table = get_trans_table(first_lv, proc->page_table);

                        int trans_seeker = 0;
                        for (; trans_seeker < trans_table->size; ++trans_seeker) {
                                if (trans_table->table[trans_seeker].v_index == second_lv) break;
                        }

                        for (int trans_move = trans_seeker; trans_move < trans_table->size - 1; ++trans_move) {
                                trans_table->table[trans_move].p_index = trans_table->table[trans_move + 1].p_index;
                                trans_table->table[trans_move].v_index = trans_table->table[trans_move + 1].v_index;
                        }
